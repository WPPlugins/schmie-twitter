...
get_option('schmie_tw_select_service_twitter')== 'true'){
			...
			$tw = new To_Twitter($msg);
			$tw_usr= get_option('schmie_tw_user');
			$tw_pwd= get_option('schmie_tw_pass');
	
			$selected_service =get_option('schmie_tw_select');		
			switch ($selected_service){
				case 'Twitter':	
					$tw->tweet_via_curl($tw_usr,$tw_pwd);
					break;
				case 'Twittermail':
	...
	public function short_url($url) {
		...
		/*$us = new Urlshortener($url);
		$new_url="";
		$selected_shortener=get_option('schmie_tw_shortener_select');
	
		switch ($selected_shortener){
			case 'is.gd':
				$new_url=$us->getShortUrlIsgd();
				break;
			case 'bit.ly':
				$new_url=$us->getShortUrlBitly(	get_option('schmie_tw_bitly_usr'),
					get_option('schmie_tw_bitly_api'));
				break;
			case 'i2h.de':
				$new_url=$us->getShortUrlI2h();
				break;
			case 'tinyurl.com':
				$new_url=$us->getShortUrlTinyurl();
				break;	
		}//switch
		return $new_url;
		*/
